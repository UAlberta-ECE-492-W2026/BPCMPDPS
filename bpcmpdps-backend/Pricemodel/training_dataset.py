...
def build_training_dataset(input_xlsx: str | Path,
    ...
    gasdf["timestamp"] = gasdf["timestamp"].astype("datetime64[ns]")
    ...
    merged_df=merged_df.merge(gasdf, on="timestamp", how="left", validate="one_to_one")
    merged_df["timestamp"] = pd.to_datetime(merged_df["timestamp"])
    now=dateutil.utils.today()
    train_df = merged_df[merged_df["timestamp"] < now.strftime("%Y-%m-%d")]
